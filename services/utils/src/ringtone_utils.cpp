...
std::map<std::string, int> RingtoneUtils::GetDefaultSystemtoneInfo()
{
    map<string, int> defaultSystemtoneInfo;
    char paramValue[SYSPARA_SIZE] = {0};
    GetParameter(PARAM_RINGTONE_SETTING_RINGTONE, "", paramValue, SYSPARA_SIZE);
    if (strcmp(paramValue, "")) {
        defaultSystemtoneInfo.insert(pair<string, int>(string(paramValue), DEFAULT_RING_TONE_SYSTEM_SIM_CARD_1_TYPE));
    }
    ...
    if (strcmp(paramValue, "")) {
        defaultSystemtoneInfo.insert(pair<string, int>(string(paramValue), DEFAULT_RING_TONE_SYSTEM_SIM_CARD_2_TYPE));
    }

    memset_s(paramValue, sizeof(paramValue), 0, sizeof(paramValue));
    GetParameter(PARAM_RINGTONE_SETTING_SHOT, "", paramValue, SYSPARA_SIZE);
    if (strcmp(paramValue, "")) {
        defaultSystemtoneInfo.insert(pair<string, int>(string(paramValue), DEFAULT_SHOT_TONE_SYSTEM_SIM_CARD_1_2_TYPE));
    }

    memset_s(paramValue, sizeof(paramValue), 0, sizeof(paramValue));
    GetParameter(PARAM_RINGTONE_SETTING_NOTIFICATIONTONE, "", paramValue, SYSPARA_SIZE);
    if (strcmp(paramValue, "")) {
        defaultSystemtoneInfo.insert(pair<string, int>(string(paramValue), DEFAULT_NOTIFICATION_TONE_SYSTEM_TYPE));
    }

    memset_s(paramValue, sizeof(paramValue), 0, sizeof(paramValue));
    GetParameter(PARAM_RINGTONE_SETTING_ALARM, "", paramValue, SYSPARA_SIZE);
    if (strcmp(paramValue, "")) {
        defaultSystemtoneInfo.insert(pair<string, int>(string(paramValue), DEFAULT_ALARM_TONE_SYSTEM_TYPE));
    }
